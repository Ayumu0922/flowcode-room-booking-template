  ...
  return (
    <PageTransition className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-6 space-y-4">
        <h2 className="text-sm font-semibold text-foreground">予約設定</h2>
        <div><label className="text-xs text-zinc-500 block mb-1">デフォルトの会議時間（分）</label>
          <input defaultValue="60" type="number" className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-sm text-foreground" /></div>
        <div><label className="text-xs text-zinc-500 block mb-1">リマインダー（分前）</label>
          <input defaultValue="10" type="number" className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-sm text-foreground" /></div>
      </div>
