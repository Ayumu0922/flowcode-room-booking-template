    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-5xl mx-auto space-y-6">
            ...
            <div key={room.id} className="grid grid-cols-[120px_1fr] divide-x divide-zinc-800 border-t border-zinc-800/50">
              ...
              <div className="relative h-12">
                {roomBookings.map((b) => {
                  ...
                  return (
                    <div key={b.id} className="absolute top-1 bottom-1 bg-purple-500/20 border border-purple-500/40 rounded text-[10px] text-purple-300 px-1.5 flex items-center overflow-hidden"
      ...
      <form onSubmit={handleSubmit} className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-6 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="会議名" required className="bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2.5 text-sm text-white placeholder-zinc-500 focus:outline-none focus:ring-1 focus:ring-purple-500/50" />
        ...
        <button type="submit" className="bg-purple-600 hover:bg-purple-500 text-white px-6 py-2.5 rounded-lg text-sm font-medium transition-colors">予約する</button>
      </form>
    </motion.div>
  );
}
